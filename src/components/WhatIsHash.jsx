...
const WhatIsHash = () => {
  ...
  // Fire event when the entire section enters the viewport
  const handleSectionView = () => {
    if (typeof window !== "undefined" && window.fbq) {
      window.fbq("trackCustom", "WhatIsHashSectionView");
      console.log("✅ Meta Pixel: WhatIsHashSectionView event sent");
    }
  };

  // Fire event when a feature card is hovered
  const handleFeatureHover = (title) => {
    if (typeof window !== "undefined" && window.fbq) {
      window.fbq("trackCustom", "WhatIsHashFeatureHover", { featureTitle: title });
      console.log(`✅ Meta Pixel: WhatIsHashFeatureHover event sent for ${title}`);
    }
  };

  // Fire event when a feature card is clicked
  const handleFeatureClick = (title) => {
    if (typeof window !== "undefined" && window.fbq) {
      window.fbq("trackCustom", "WhatIsHashFeatureClick", { featureTitle: title });
      console.log(`✅ Meta Pixel: WhatIsHashFeatureClick event sent for ${title}`);
    }
  };
  ...
  return (
    ...
    >
      <div className="max-w-screen-2xl mx-auto text-center mb-12">
        <h2 className="text-3xl sm:text-4xl md:text-5xl mb-4 uppercase">WHAT IS HASH?</h2>
      ...
      <div className="w-full max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-6">
        {features.map((feature, idx) => (
          <div
            key={idx}
            className="relative flex items-center justify-center px-2 cursor-pointer focus:outline-none"
